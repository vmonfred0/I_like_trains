...
class Agent(BaseAgent):
    def my_train(self) -> dict[str, any]:
        return self.all_trains[self.nickname]

    def other_trains(self) -> list[str]:
        the_trains = []
    ...
    def my_direction(self) -> Move:
        return Move(tuple(self.my_train()["direction"]))

    def my_wagons(self) -> list[tuple[int, int]]:
        return self.my_train()["wagons"]

    def my_speed(self, wagon: int) -> float:
        return 0.95**wagon

    def my_possible_moves(self) -> list[Move]:
        return [
            self.my_direction(),
            self.my_direction().turn_left,
            self.my_direction().turn_right,
        ]

    def my_train_coordinates(self, name: str) -> tuple[int, int]:
        return tuple(name["position"])

    def safe_coordinates(
        self, width: float, height: float, cell_size: float
    ) -> tuple[float, float]:
        # retourne les coordonnées valables pour demeurer sur la grille du jeu
        return width - cell_size, height - cell_size

    def get_occupied_cases(self) -> list[tuple[int,int]]:
    ...
    def get_coordinates_around_players(self, occupied:list)-> list[tuple[int,int]]:
    ...
    def grid(self)->list[list[int]]:
        ...
        discrete_grid = [
            [0 for j in range(self.game_width // self.cell_size)]
            for i in range(self.game_height // self.cell_size)
        ]
    ...
    def get_coordinates_delivery_zone(self) ->  list[tuple[float,float]]:
    ...
    def get_coordinates_around_delivery_zone(self, delivery_zone:list)-> list[tuple[float,float]]:
        a = self.cell_size
    ...
    def dijkstra(self, start: tuple[float,float], goal: tuple[float,float], wagons: tuple[float,float])-> tuple[int,any,float]:
        ...
        # prend en entrée la case de départ (start), la case visée (goal)
        ...
        ]  # on initialise la pondération avec infini pour chaque case
        ...
        sx, sy = (
            sx0 // cell,
            sy0 // cell,
        )  # coordonnées adaptées à la taille des cellules
        ...
        directions = [
            ...
            (0, -1),
            (0, 1),
        ]  # à différencier des directions usuelles
        while queue:
            ...
            for dx, dy in directions:
                nx, ny = x + dx, y + dy
                if (
                    0 <= nx < rows and 0 <= ny < cols and grid[nx][ny] == 0
                ):  # condition 0 signifie que la case est libre, pas d'obstacles
                    new_dist = dist + 1
    ...
    def calculate_best_move_for(self, kind_of_move: list, name:str):
        ...
        for i in range(len(kind_of_move)):
            ...
            if kind_of_move == self.get_coordinates_delivery_zone():
                ...
                case = x, y
                a = len(self.my_wagons())**2

            path_length, first_move, time_spent = self.dijkstra(
                coordinates, (x, y), wagons
            )
    ...
    def calculate_absolute_best_move(self, name):
        ...
        for i in kind_of_moves:
            time, value, move, case_chosen, path_length, s_m, o_b = (
                self.calculate_best_move_for(i, name)
            )
            if value > optimum:
    ...
    def prediction_of_the_other(self):
        a, best_move, my_case_chosen, my_time, c, secondary = (
            self.calculate_absolute_best_move(self.nickname)
        )
        if c == self.passengers and self.other_trains():
            for i in self.other_trains():
                if self.all_trains[i]["alive"]:
                    o, b, i_case_chosen, i_time, m, s = (
                        self.calculate_absolute_best_move(i)
                    )
                    if i_case_chosen == my_case_chosen and i_time < my_time:
                        print('SKIBIDI HOBIDI SECONDARY')
                        return secondary
        print('ok')
        return best_move
    ...
    def bastard_move(self):
        # BAT LES COUILLES
        a = self.get_coordinates_around_delivery_zone(
            self.get_coordinates_delivery_zone()
        )
    ...
    def get_authorized_moves(self, position, safe, direction, a):
        ...
        for i in possible_moves:
            ...
            if (
                0 <= sum_x <= x_safe
                and 0 <= sum_y <= y_safe
                and ([sum_x, sum_y] not in self.get_occupied_cases())
            ):
                if a > 1:
                    next_move = self.get_authorized_moves(
                        (sum_x, sum_y), safe, direction, a - 1
                    )
                    if not next_move:
                        continue
    ...
    def get_move(self):
        ...
        print('\d')
        coordinates = self.my_train_coordinates(self.my_train())
        s_coordinates = self.safe_coordinates(
        ...
        authorized_moves = self.get_authorized_moves(
            coordinates, s_coordinates, direction, 2
        )
